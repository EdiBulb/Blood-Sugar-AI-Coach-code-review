...
export default function WeeklySummary(){
  const [loading, setLoading] = useState(false); // 데이터를 불러오는 중인지 여부(버튼을 Generating...으로 바꾸는데 사용)
  const [report, setReport] = useState(null); // 받아온 주간 리포트 데이터 저장

  async function generate(){
    setLoading(true); // 로딩 시작
    try{
      const { data } = await axios.get("/api/summary/weekly"); // GET 요청
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow">
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          disabled={loading} // 로딩 중에는 비활성화(disabled)되고 "Generating..." 테스트로 바뀜
          className="px-3 py-1 rounded bg-blue-600 text-white disabled:opacity-50"
        >
          {loading ? "Generating..." : "Generate"}
        </button>
      </div>
      {/* 조건부 렌더링: report가 있을 때만 이 부분이 보여짐 */}
      {report && (
        <div className="mt-3 space-y-2">
          <div className="text-sm opacity-70">7-day avg: <b>{report.avg} mg/dL</b></div>
          {/* 조건부 렌더링: spike.delta가 있을 때만 표시 */}
