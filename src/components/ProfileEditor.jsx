...
export default function ProfileEditor(){
  ...
  },[]); // 빈 []은 uesEffect가 처음 한 번만 실행되도록 설정

  // 커스텀 set(k,v)함수
  function set(k,v){ setForm(prev=>({...prev, [k]:v})) } // 기존 값 유지하면서 특정 필드만 바꿈

  async function save(){
    await axios.put("/api/profile", { // 폼 데이터를 /api/profile에 PUT 요청으로 저장함
      goals: form.goals, 
      diet: form.diet, 
