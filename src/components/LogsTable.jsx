...
export default function LogsTable({ logs = [], onChanged }) {
  ...
  async function onDeleteButton() {
    ...
    if (selected.size > 0) {
      ...
      await axios.delete("/api/logs", {
        data: { username: user.username, ids }, // 유저명도 함께 전송
      });
      setSelected(new Set());
      setSelectMode(false);
      onChanged?.(); // 새로고침
  ...
  // 버튼 라벨: Delete, Done 등 상태에 따라 변경
  const deleteLabel = !selectMode
    ? "Delete"
    : selected.size > 0
    ? "Delete"
    : "Done";
      ...
      <table className="min-w-full text-sm">
        <thead>
          <tr className="text-left border-b">
            {selectMode && !isGuest && <th className="py-2 pr-2 w-8"></th>}
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.id} className="border-b last:border-none align-top">
              ...
              <td
                className={`py-2 pr-4 font-semibold ${statusClassByMmolFromMg(
                  row.value,
                  row.mealState || "Fasting"
                )}`}
              >
                {row.mmol.toFixed(1)}
              </td>
