// ✅ 수정된 LogForm.jsx (guest 계정 제한 + username 포함)
...
export default function LogForm({ onSaved, setCoachMessage }) {
  ...
  async function handleSubmit(e) {
    ...
    await axios.post("/api/logs", {
      username: user.username, // 사용자 이름 포함!
      date: dateISO,
      timeSlot,
      value: mgdl,
      note: showNote ? note : "",
    ...
    // 2) AI 코치 메시지 받아옴
    const { data } = await axios.post("/api/coach", {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow">
      <h2 className="text-xl font-semibold mb-3">
        Today’s blood sugar{" "}
        {isGuest && (
          <span className="text-red-500 text-sm">(Demo mode: read-only)</span>
        )}
      </h2>
      ...
      <div className="flex gap-3 items-end flex-wrap">
        ...
        {/* 저장 */}
        <button
          className="ml-auto bg-blue-600 text-white px-4 py-2 rounded-lg hover:opacity-90"
          disabled={isGuest}
        >
          Save & Get Tip
        </button>
      ...
      {showNote && (
        <div className="mt-3">
          <label className="block text-sm mb-1">
            Memo (Food/Exercise/Fasting etc)
          </label>
          <textarea
